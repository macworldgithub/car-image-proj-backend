...
// Allow frontend running on file:// or other origins
app.use(cors({
  origin: true
}));

// Simple health route
app.get("/", (req, res) => res.send("OpenAI composite backend running"));
 ...
 */
// keep your existing imports, app, upload, cors, dotenv config, etc.
// Replace only the /api/composite handler with this code.
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Always append car as the first image and background as second.
      ...
      // Optionally append logo as third image.
      if (logoFile) {
        form.append("image[]", logoFile.buffer, { filename: logoFile.originalname });
      }

      // Build explicit prompt describing expected behavior.
      // Be clear about the conditional behavior when a logo is provided.
      const promptLines = [
      ...
      form.append("n", "1");
      form.append("size", "1024x1024"); // optional: change if you want
